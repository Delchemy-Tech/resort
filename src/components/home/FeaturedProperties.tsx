...
const FeaturedProperties: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (featuredProperties.length === 0) {
      dispatch(fetchFeaturedProperties(12)); // Get 12 properties for the gallery
    }
  ...
  // Use featured properties data or fallback to mock data if none available
  const displayProperties = featuredProperties.length > 0 
    ? featuredProperties.map((property: any) => ({
        id: property.id,
        src: property.image || fallbackImage,
        alt: property.title || 'Resort Property'
      }))
    : [
        // Fallback gallery images (keeping original as backup)
        {
          id: '1',
  ...
  const getImagesForBreakpoint = (count: number) => displayProperties.slice(0, count);
    ...
    <section className="bg-white">
      ...
      <div className="w-full">
        ...
        <div className="sm:hidden grid grid-cols-2 gap-0">
          {getImagesForBreakpoint(4).map((image: any) => (
        ...
        <div className="hidden sm:grid md:hidden grid-cols-3 gap-0">
          {getImagesForBreakpoint(6).map((image: any) => (
            <div key={image.id} className="relative group overflow-hidden">
              <Image 
                src={failedImages.has(image.id) ? fallbackImage : image.src} 
                alt={image.alt}
        ...
        <div className="hidden md:grid lg:hidden grid-cols-4 gap-0">
          {getImagesForBreakpoint(8).map((image: any) => (
            <div key={image.id} className="relative group overflow-hidden">
              <Image 
                src={failedImages.has(image.id) ? fallbackImage : image.src} 
                alt={image.alt}
        ...
        <div className="hidden lg:grid grid-cols-6 gap-0">
          {getImagesForBreakpoint(12).map((image: any) => (
            <div key={image.id} className="relative group overflow-hidden">
              <Image 
                src={failedImages.has(image.id) ? fallbackImage : image.src} 
                alt={image.alt}
